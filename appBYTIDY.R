...
fdat        <-      read.delim("24Jan8/2021_July19_fdata.txt",
...
header <- dashboardHeader(title =  span("Chemogenomic Profiling COVID19 Humanscreens",
...
background-color: #000066 !important; color: #fff font-weight:bold}
...
// Function to position draggable, place on current mouse coordinates
 Shiny.addCustomMessageHandler ('eplaceDraggable',
function (message) { var element = $('#eclick_info').parent();
...
tags$style(HTML(".selectize-input{white-space:
    nowrap;
...
fluidRow(column(
      width = 12,
        ...
        h5(
          ...
          "click OR drag to select screens of interest;
corresponding profiles can be found
...
status = "primary", solidHeader = T, width = "80%" )) #column ok

fluidRow(

        box(title = "Experimental detail;
          select row to view corresponding HOP chemogenomic profile",
...
fluidRow(
        box(
        title = "Cofit genes, select row to view
          corresponding gene fitness profile",
...
fluidRow(
  ...
  fluidRow(
        ...
        title = "GO enrichment network:
            select a node to view details; right click to save image",
        width = 8,
status = "primary", solidHeader = T, width = "100%", height = 190)
...
tabItem(
  ...
  box(title = "Select screen:",
          ...
          selectizeInput('cmpSERV', label = NULL, choices = phop$name,
            multiple = F, selected = "azithromycin_1.75mM",
...
actionLink(
 ...
 status = "primary", solidHeader = T, width = "100%", height = 190) #colname ok
 uiOutput("ylimits")
      ),# fluidrow ok
...
sigNetworkModuleUI("sigNetworkModule1")
 ...
 status = "primary", solidHeader = T, width = "100%", height = 190)
  ...
  tabItem("goenrich",
        ...
        fluidRow(visNetworkModuleUI("visNetwork1"))#fluid row) #rowtab ok
    )))
)
...
observeEvent(input$site, {
  ...
  if(site == "marjhop") {
    choices = phop$name} else if(site == "marjhiphop"){
    choices = pmarjhip$name} else if(site == "elke"){
    choices = pelke$name}

  if(site == "marjhop") {

    selected = "24Jan05:methyl-methanesulphonate-mms_800uM"} else if(site == "marjhiphop"){
...
output$hiphoppanel  = renderUI({
 ...
 fluidRow(
  ...
  box(title="HOP genes:
    mouse click on row links to gene fitness profile;
...
options = list(cancel = ".shiny-input-container"))),
        ...
        fluidRow(
          box(title="HIP genes: mouse
              click on row links to gene fitness profile",

               dataTableOutput("hiptab",width = "100%"),

              status = "primary", solidHeader = T,width = 6),

                box(title="HOP genes: mouse
                    click on genename links to SGD",
 ...
 box(title="download HOP:",
  ...
  status = "primary", solidHeader = T,width = 6)),
        ...
        fluidRow(
          ...
          pvalue, PubChem ID (CID) link, Mechanism of

          Action (MoA) link, indication & mechanism",
...
and corresponding GO enrichments in next tab")),
 br(),
  dataTableOutput('coInhib'),status = "primary",
solidHeader = T,width = 12)
...
fluidRow(
 ...
 box(title = "HOmozygous Profile (HOP):
     mouse click on points to view gene description",status = "primary",
                   plotOutput("nfd", width = "90%",height = 700,
...
box(title="HOP genes: mouse click on row
    links to gene fitness profile; mouse click on genename links to SGD"dataTableOutput("hoptab"),
  ...
  observeEvent(cmpRETURN(),
               {
                 ...
                 if(cmpMOD %in% phop$name){choices = phop$name}else if(cmpMOD %in% pmarjhip$name){
                   ...
                   if(cmpMOD %in% phop$name){site = "marjhop"} else if(cmpMOD %in% pmarjhip$name){
                        ...
                        site = "marjhiphop"}else if(cmpMOD %in% pelke$name){site = "elke"}
...
opts - list(className = 'dt-left', targets = c(0, 1, 2, 3)),
 ...
 observeEvent(input$resetCmp,{
  ...
  if(site == "marjhop"){
   ...
   updateSelectizeInput(session,"cmpSERV",
      ...
      selected = "24Jan05:methyl-methanesulphonate-mms_800uM",server = T)
...
output$fiToutput = renderUI({
  ...
  HTML("<h5><b>Top: 1000 genes ordered by cofitness"),

  HTML("<h5><b>Bottom: Corresponding cofitness rank from 1 to 1000"),
  ...
  HTML("<h5><b>scale
       marks # of strains used in GO enrichment"),
 ...
 br(),br(),
  ...
  HTML("<h5><b>Cofitness values:</b></h5>"),
      ...
      status = "primary",
  solidHeader = T, width="100%", height = 650)))
...
observeEvent(input$corrFIT,{
     ...
     w1 = slid(x =input$corrFIT,y
               = input$scoreFIT, mfit = mfit)
     score1 = w1$corr[2]
     ...
     print(c("scorefit",score,
             input$scoreFIT,scoreVal$current))
  ...
  if( is.null(scoreVal$current)){
   ...
   } else if(!is.null(scoreVal$current)& as.numeric
             (input$scoreFIT) != score){
    ...
    observeEvent(input$scoreFIT,{
      ...
      w1 = slid(x =input$corrFIT,
      y= input$scoreFIT, mfit = mfit)
...
output$mxFIT <- renderUI({
  ...
  HTML("<h5><b>Finds the cofitness/cofitness rank that
       maximizes
       the number of GO enrichment terms
       for the top 1000 cofit
 ...
 between ranks tested.<h5><b>"),
  ...
  fluidRow(
    ...
    column(width = 12,align = "center",
     ...
     prettyRadioButtons(inputId = "stepsize",label =
     "stepsize",
...
selected = 50, inline = T)),
    ...
    HTML("<h5><b>&nbsp&nbsp&nbsp&nbsp&nbspSet FDR
         Set threshold (default = 0.1)</b></h5>"),
   ...
   column(width = 12,align = "left",
         ...
         actionButton("rankGO", "GO!", class = "btn
                      -success")),

   column(width = 12, align = "left",
         tags$head(tags$style(HTML("<h5><b>
                                   l
...
goEnrich <- reactive({
     ...
     validate(need(length(wfit)!=0, message = "No
                   CoFitness scores above threshold"))

     req(length(wfit)!=0)

     qFIT = compRANK(mfit,coln = 2,
                     rank  = input$scoreFIT)
     ...
     network = runGOENRICH(
       fdrThresh = FDR, minSetSize = 5,curr_exp =
         curr_exp,

        score = qFIT,bp_path =  "2023_January30_GOBP.RDS",
        go_path = "2023_January30_GOID_GOBP_SGD.txt")

     validate(need(!is.null(network$enrichInfo),
     message = "No GO enrichment,
    ...
    if(!is.null(enrichInfo)) gseaMAX$goterms = nrow
    (enrichInfo)
 ...
 output$network_Proxy <- renderVisNetwork({
    ...
    visOptions(
      ...
      highlightNearest = list(enabled = T,
          ...
          nodesIdSelection =
        list(enabled = TRUE, values = names,
    ...
    Shiny.onInputChange('current_node_id', nodes.nodes);;}")
      ...
      highlightNearest = list(enabled = T,
          ...
          nodesIdSelection = list(enabled = TRUE, v
          values = names,
          ...
          selectedBy = list(variable="FDR",
            ...
            font-size: 18px; color: #000066;border:
            3px solid #4d88ff;')) %>%

        visIgraphLayout(type = "full") %>%

        visEvents(select = "function(nodes) {
        Shiny.onInput;}") Change('current_node_id', nodes.nodes);
...
output$goEnrichBarPlot <- renderPlot({
  ...
  w = which(vis$nodes$id %in% c
            (input$network_Proxy_selected))
  ...
  barplot(d$score, names.arg = d$gene, las = 1,

  horiz = ,cex.axis = 0.8, cex.lab = 1.1, col ="navy", xlab = "median fitness defect score")
...
hip
  ...
  updateTextInput(session, inputId = "gene", label = "", value = df$gene[row])
...
essPlot = function(){
  validate(
    need(input$cmpSERV,message =
           "please select condition"))
  cond1 = input$cmpSERV
  if(input$site == "marjhiphop") {x = de3}
...
output$hoptab = DT::renderDataTable({
      ...
      if(input$site == "marjhop"){mat = d3[,phop$name]} else if(
        input$site == "marjhiphop"){mat = dn3} else (mat = delke)
...
output$nfd = renderPlot({
  ...
  if(input$site == "marjhiphop") {x = dn3}
...
if(input$site == "marjhiphop"){
  coln = colnames(x)[wx]
  coln   = str_wrap(coln,width=30,whitespace_only = F)
  colnames(x)[wx] = coln
   }
  req(length(wx) > 0)
  p10(x,wx,pch = 17, ylim = input$ylim,
      sig = input$scorethreshSERV)
 ...
 nonPlot = function(){
   ...
   cond1 = input$cmpSERV
      ...
      if(input$site == "marjhop"){
        ...
        x = d3[,phop$name]} else if(

        input$site == "marjhiphop")
          {x = dn3} else (x = delke)
             ...
             fluidRow(
               ...
               column(width = 12,align = "center",
                 ...
                 actionLink(
                   ...
                   label = "reset", style = "height: 100px; width: 150px;
                    ...
                    column(width = 12,align = "center",
                      ...
                      actionLink(
                        ...
                        inputId = "eorig",
                          ...
                          label = "reset", style = "height: 100px;

                          width: 150px; font-size:120%; text-align:center",
