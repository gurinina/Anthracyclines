...
output$fiToutput = renderUI({
  ...
  tagList(
    fluidRow(
      ...
      HTML("<h5><b> Click on arrow to animate GO enrichment
           dependency on cofitness rank.</b></h5>"),

      HTML("<h5><b>scale marks # of strains used in GO enrichment"),
      ...
      choices = st,grid =T,animate = T),
        ...
        HTML("<h5><b>Set FDR threshold for GO enrichment:</b></h5>"),
...
nonPlot = function(){
  ...
  cond1 = input$cmpSERV
      ...
      if(input$site == "marjhop"){x = d3[,phop$name]} else if(
        input$site == "marjhiphop"){x = dn3} else (x = delke)
...
observeEvent(input$gene,{
      whop = which(rownames(xcofit)%in% toupper(input$gene))
      whh = which(rownames(xfithh)%in% toupper(input$gene))
      we = which(rownames(xefit)%in% toupper(input$gene))

      if(length(whop)!=0){xfit = xcofit}
      if(length(whop)==0 & length(whh)>0 ){xfit = xfithh}
      if(length(we)!=0){xfit = xefit}

      if(length(whop)!=0){xpit = xpfit}
      if(length(whop)==0 & length(whh)>0 ){xpit = xpithh}
      if(length(we)!=0){xpit = xefitpv}
      ...
      df = data.frame(gene = rownames(d), cofit = d[rownames(d),1],pvalue = p[rownames(d),1],stringsAsFactors = F)
