...
header <-  dashboardHeader(title =  span("Chemogenomic Profiling          COVID19", style = "font-weight:bold;font-size:18px; text-align: right   ;"), titleWidth = 1200)
    ...
    box(title = "Select input datasets:",
      ...
      prettyRadioButtons("site", label = NULL,
        ...
        choices = c(
          ...
          "2021:2024 HIPHOP Marjan" = "phiphop",
          "2020:2024 HOP Marjan" = "phop"),
  ...
  column(width = 9,
    ...
    box(title = "Select screen:",
      ...
      selectizeInput('cmpSERV', label = NULL,
        ...
        choices = phop$name, multiple = FALSE, selected = "comptothecin_360uM",
...
observeEvent(cmpRETURN(), {
  ...
  if(cmpMOD %in% phop$name){choices = phop$name} else if(cmpMOD %in% phiphop$name){

    choices = phiphop$name}

  if(cmpMOD %in% phop$name){site = "phop"} else if(cmpMOD %in% phiphop$name){
  ...
  req(site)
    ...
    updatePrettyRadioButtons(session,"site", label = NULL,
      c("2021:2024 HIPHOP Marjan" = "phiphop",
        "2020:2024 HOP Marjan" = "phop"), selected = site)
...
observeEvent(input$site, {
  ...
  if(site == "phop") {
    ...
    choices = phop$name} else if(site == "phiphop"){

    choices = phiphop$name}

  if(site == "phop") {

    selected = "camptothecin_360"} else if(site == "phiphop"){
...
observeEvent(input$resetCmp,{
  ...
  updateSelectizeInput(session,"cmpSERV",
   ...
   selected = "comptothecin_360uM",server = T)

  } else if(site == "phiphop"){

  updateSelectizeInput(session,"cmpSERV", label = NULL,
   choices = phiphop$name,
   selected = "camptothecin_360uM",server = T)
...
observeEvent(input$plot_click, {
  ...
  if(length(whop) > 0) {x = d3} else if(length(wboth) > 0) {x = dn3}
...
if(length(whop) > 0) {site = "phop"}

if(length(wboth) > 0) {site = "phiphop"}
...
choices = phop$name} else if(selected%in% phiphop$name) {
choices = phiphop$name}
...
if(input$site == "phiphop") {xfit = xfithh}
if(input$site == "phop") {xfit = xcofit}

if(input$site == "phiphop") {xpv = xpithh}
if(input$site == "phop") {xpv = xpfit}
...
OutEnrich = reactive({
  ...
  format(round(enrich[,c("querySetFraction","geneSetFraction", "foldEnrichment")],2),nsmall = 1,scientific = FALSE)
...
output$EnrichTable = DT::renderDataTable({
  ...
  df =  DT::datatable(out, options = opts,rownames = FALSE, escape = FALSE, selection = "single") %>%
...
},escape = FALSE,rownames=FALSE,options=list(pageLength=10),selection = "single",server = FALSE)
...
observeEvent(input$coFitNess_rows_selected, {
  ...
  xfit = outFit$dFit
   ...
   delay(200,updateTextInput(session, inputId = "gene", label = NULL, value = xfit$gene[row]))
...
output$downloadcofitness <- downloadHandler(
  ...
  paste0("CoFit:",input$gene, "_",Sys.Date(), ".txt")
      ...
      content = function(file) {
        ...
        write.table(as.data.frame(outFit$dFit), file, row.names = FALSE,sep="\t",quote=FALSE)
      }
    )
...
outcoInhib = reactive({
  ...
  if(input$cmpSERV%in% phop$name) {
            ...
            xpv =  xpval
            pdata = phop
      } else if(input$cmpSERV%in% phiphop$name){
            xcov = xcohh
            xpv = xpvhh
            pdata = phiphop}
      ...
      df = data.frame(screen = rownames(d)[1:nrow(d)],
                      CoI = d[1:nrow(d),1],pvalue = p[rownames(d),1],stringsAsFactors = FALSE)
...
output$gotermTable <- DT::renderDataTable({
    ...
    datatable(t, width=220,caption = htmltools::tags$caption(term,
      ...
      options=list(paging=FALSE,scrollY=FALSE,dom="t",scroller=FALSE,searching=FALSE,ordering=FALSE,rowCallback = JS(
                "function(row, data) {",
...
output$BarPlot <- renderPlot({
  ...
  if(nrow(d)> 10) d = d[1:10,]
     ...
     barplot(d$score,names.arg = d$gene,las=1,horiz=TRUE,cex.axis = 0.8, cex.lab = 1.1, col="navy",xlab = "median fitness defect score")
...
output$hiptab = DT::renderDataTable({
  ...
  hip = geneAnno(mat,fdat = fdat,sgdlink = TRUE,cmp = input$cmpSERV,
...
outhip = reactive({
  ...
  delay(200,updateTextInput(session, inputId = "gene", label = NULL, value = df$gene[row]))
...
output$downloadhip <- downloadHandler(
  ...
  content = function(file) {
    write.table(as.data.frame(outhip()), file, row.names = FALSE,sep="\t",quote=FALSE)
  }
  )
...
output$hoptab = DT::renderDataTable({
    ...
    if(input$site == "phop"){mat = d3[,phop$name]} else if(
    input$site == "phiphop"){mat = dn3}
...
output$downloadhop <- downloadHandler(
  ...
  content = function(file) {
    write.table(as.data.frame(outhop()), file, row.names = FALSE,sep="\t",quote=FALSE)
  }
  )
...
output$nfd = renderPlot({
  ...
  if(input$site == "phiphop") {x = dn3}
  if(input$site == "phop") {x = d3}
...
nonPlot = function(){
  ...
  cond1 = input$cmpSERV
    ...
    if(input$site == "phop"){x = d3[,phop$name]} else if(
    input$site == "phiphop"){x = dn3}
...
observeEvent(input$resetscore, {
  ...
  updateSliiderInput(session, "scorethreshSERV", label = NULl,
    value = 1.0, step =0.1,
    min = 0, max = 5.0)
