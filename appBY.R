...
fluidRow(
    ...
    box(title = "Experimental detail; select row to view corresponding
     ...
     status = "primary", solidHeader = TRUE, width = 12, height = 250)), # ok
...
fluidRow(
  ...
  column(width = 6,
   ...
   #fluidRow
# geneinfo tsbpyd vofiyt mxxfit Gotersb Gotable leadingedge EnrichTable ownload CoFit
...
fluidRow(
    ...
    solidHeader = TRUE, background = "navy", status = "primary", height = 200),
    ...
    solidHeader = TRUE, status = "primary", height = 580), # ok
  ...
  fluidRow(
    ...
    status = "primary", solidHeader = TRUE, width = "100%", height = 150), # box ok
...
output$sidebarPanel <-renderMenu({
  ...
  menuItem("HOmozygous Profiles", tabName="hiphop",icon = icon("bullseye"), selected = 1),

  menuItem('GO enrichments', tabName = 'goenrich', icon = icon('dna')),

  menuItem('Signatures', tabName = 'signature', icon = icon('database')),

  menuItem("Gene fitness profiles", tabName="genebyscreen", icon = icon("chart-line")))})} else {

output$sidebarPanel <-renderMenu({

  sidebarMenu(id = "tabs", selected = HTML("HaploInsufficiency Profiles<br/>&nbsp&nbsp&nbsp&nbsp&nbsp&nbspHomozygous Profiles"),

  menuItem(HTML("HaploInsufficiency Profiles<br/>&nbsp&nbsp&nbsp&nbsp&nbsp&nbspHomozygous Profiles"),
...
w1 = slid(x =input$corrFITRUE,y = input$scoreFITRUE, mfit = mfit)
...
#
#
    observeEvent(input$scoreFITRUE,{
...
observeEvent(input$rankGO,{
  ...
  net = lapply(dFITRUE,runGOENRICH,fdrThresh = FDR, minSetSize = 5,
...
if(length(ylims) == 2){
  column(width = 3,
    ...
    box(title= "HOP y-axis range:",
       ...
       fluidRow(column(width = 12,
         sliderInput("ylim", label = NULL, min = elims[1],
           max = elims[2], value = c(-0.5,elims[2])))),
