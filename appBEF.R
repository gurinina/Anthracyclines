...
fluidRow(
  ...
  width = "100%", height = 250))), # fluidRow ok
...
fluidRow(
  ...
  column(width = 3,
    ...
    box(title = "Select input datasets:",
      ...
      prettyRadioButtons("site", label = NULL,
        choices = c("2021:2024 HIPHOP" = "phiphop",
                    "2020:2024 HOP" = "phop"),
  ...
  column(width = 9,
    ...
    status = "primary", solidHeader = TRUE,
      width = "100%", height = 160))), # fluidRow ok
...
fluidRow(
  ...
  column(width = 3,
    ...
    status = "primary", solidHeader = TRUE,
      width = "100%", height = 200)), # column ok
...
#   site = input$site
  ...
  updatePrettyRadioButtons(session,"site", label = NULL,
    c("2021:2024 HIPHOP" = "phiphop",
      "2020:2024 HOP" = "phop"), selected = site)
...
selected = "camptothecin_360uM"} else if(site == "phiphop"){
...
opts = list(pageLength = 10, autoWidth = FALSE, scrollX = FALSE,
  columnDefs = list(
...
df =  DT::datatable(df, options = opts,rownames = FALSE,
...
hideTooltip <- function( hide ){
  ...
  session$sendCustomMessage(type = 'ehideDraggable',
...
}, escape = FALSE, rownames = FALSE, selection = "single",server = FALSE)
...
output$geneinfo  <- renderDataTable({
  ...
  info = FALSE, scrollX = TRUE, scrollY=TRUE, scrollCollapse=TRUE,
...
HTML("<h5><b>Corresponding cofitness rank:</b></h5>"),
  ...
  sliderInput("scoreFIT",label = NULL,
...
tagList(
  ...
  box(title = "Optimizing GO enrichment:",
    ...
    column(width = 12,align = "center",
      ...
      sliderInput("fdrRANK",label = NULL, min = 0, max = 0.5,
        value = 0.2, step = 0.1)),
...
DT::datatable(out, options = opts, rownames = FALSE,
  escape = FALSE, selection = "single") %>%
...
hip = geneAnno(mat,fdat = fdat,sgdlink = TRUE,
...
opts = list(pageLength = 5, autoWidth = TRUE, scrollX = TRUE,
  columnDefs = list(list(width = "80px", targets = c(-2,-3))))

hip

},escape = FALSE, options = opts, rownames = FALSE)
...
if(input$site == "phop"){
  mat = d3[,phop$name]} else if(input$site == "phiphop"){
  mat = dn3}
...
if(input$site == "phop"){
  x = d3[,phop$name]} else if(input$site == "phiphop"){
  x = dn3}
...
if(length(wne) > 0) {
  ...
  ylims = c(floor(min(x[wne,wcoln,drop=F],na.rm = TRUE)),
...
elims = c(floor(min(x[we,wcoln,drop=F],na.rm = TRUE)),
...
xne = xinput()
ylims = c(floor(min(xne[wne,wcoln,drop=F],na.rm = TRUE)),
...
observeEvent(input$eorig,{
  ...
  if(length(we) > 0) {elims = c(floor(min(xe[we,wcole,drop=F],na.rm = TRUE)),
